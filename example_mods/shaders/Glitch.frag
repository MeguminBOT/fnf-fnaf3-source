...
float rand(vec2 p)
{
    return fract(sin(dot(p, vec2(iTime * 12.9898, iTime * 78.233))) * 43758.5453);
}
...
void main()
{
    ...
    float colR = texture(iChannel0, vec2(uv.x + shift, uv.y)).r * (1.0 - shift);
    float colG = texture(iChannel0, vec2(uv.x - shift, uv.y)).g * (1.0 - shift) + rand(id) * shift;
    float colB = texture(iChannel0, vec2(uv.x - shift, uv.y)).b * (1.0 - shift);
